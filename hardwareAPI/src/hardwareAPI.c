 ...
 * $Log: hardwareAPI.c,v $
 * Revision 1.2.1 2002/09/11 12:50 thai
 *	  Changed write, read to send, recv for proper socket communication
 * Revision 1.2  2002/09/09 18:00 thai
 *    Inserted proper include files in order to make it runs in windows system
...
void initHW(char *hostname, int port)
{
  ...
  struct hostent *q;
	...
	if ( err != 0 ) {
		fprintf(stderr, "Error in starting up sockets. Error code: %d\n", err);
		exit(-1);
	}
 #endif
...
EventType waitForEvent(EventDesc *event)
{
  ...
  while (1) {
	...
	exit(-1);
      ...
      //
    if ((count = recv(hwd, wPtr, freeSpace,0)) <= 0) {
